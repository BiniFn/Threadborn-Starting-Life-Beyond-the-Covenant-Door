...
async function refreshSession(sessionId) {
  ...
  const expiresAt = new Date(Date.now() + SESSION_TTL_MS);
  const { rows } = await pool.query(
    "update sessions set expires_at = $2, updated_at = now() where id = $1 returning expires_at",
    [sessionId, expiresAt],
  );
  return rows[0]?.expires_at || expiresAt;
}
...
function validateCsrf(req, session) {
  ...
  if (
    typeof token !== "string" ||
    token.length < 10 ||
    token.length !== session.csrf_token.length
  )
    return false;
  try {
